...
Usage:
  ./generator <video> <output> <interval> <width> <height> <columns> [<parallelism>]
  ./generator (-h | --help)
  ./generator --version
    ...
    if os.path.isdir(input_path):
        ...
        for file_name in os.listdir(input_path):
            ...
            if os.path.isfile(file_path):
                ...
                Xinterval = round(duration)/16
                division = width/320
                Xheight = round(height/division)

                Xsize = (int(320), int(Xheight))

                Xcolumns = 4

                work_queue.put((file_path, single_output_path, Xinterval, Xsize, Xcolumns,))
                work_units += 1
    else:
        video = cv2.VideoCapture(input_path)
        ...
        work_queue.put((input_path, output_path, Xinterval, Xsize, Xcolumns,))
        work_units += 1
...
def generate_frames(file_name, video_file_clip, interval, output_prefix, size):
    ...
    for i in range(1, round(duration/interval)*round(interval), round(interval)):
